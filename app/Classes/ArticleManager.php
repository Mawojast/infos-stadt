    ...
    public function removeDoubleArticles(): void
    {
        ...
        for ($i = 0; $i < count($searchForSameArticles)-1; $i++) {
            ...
            for ($j = $i + 1; $j < count($searchForSameArticles); $j++) {
                ...
                if (
                    $searchForSameArticles[$i]->imagePath == $searchForSameArticles[$j]->imagePath
                    || $searchForSameArticles[$i]->title == $searchForSameArticles[$j]->title
                    || $searchForSameArticles[$i]->description == $searchForSameArticles[$j]->description
                ) {
